...
def find_path(maze_array, start_coord, end_coord):
    ...
    #A dictionary(coord) with keys as the coordinates of the maze and values as a list of all the subsequent possible coordinates that the ball can travel in as the values.
    i=0
    while i<10:
        j=0
        while j<10:
    ...
    #Applying DFS algorithm to find shortest path in the maze beginning from the start coordinate.
    path=[]
    visited= set()
    ...
    if end_coord not in path:
      #Declaring path as None if a path doesn't exist for the given start and end coordinates.
      path= None
    else:
      end_ind= path.index(end_coord)
      #Deleting the path after it reaches the end coordinate.
      del path[end_ind+1:]

      #Removing those coordinates from the path which do not give the shortest path.
      i=1
      count=0
      k= i-1
      while i < len(path):
        ...
        if count == len(coord[path[k]]) and i!=1:
            #Removing coordinates whose coord values do not contain the element path[i]
            path.remove(path[k])
            i= i-1
            k= k-1
        else:
...
def read_start_end_coordinates(file_name, maze_name):
    ...
    for i in a:
      #Extracting the start and end coordinates for which the maze_name string mathces with the string in the JSON file
